...
// Interactive Lightbox component for the visual area
...
}// High-fidelity interactive premium image frame with 3D mouse tilt and cursor light glow
...
function PremiumImageFrame({ children, aspect = "aspect-square", title, subtitle, onClick, theme }: PremiumImageFrameProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Mouse coordinates relative to the frame container
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    setCoords({ x, y });

    // Precise, subtle 3D tilt effect (max 2 degrees to avoid distracting from content)
    const centerX = rect.width / 2;
    const centerY = rect.height / 2;
    const rotateX = ((centerY - y) / centerY) * 1.5; // max 1.5 degrees
    const rotateY = ((x - centerX) / centerX) * 1.5; // max 1.5 degrees
  ...
  if (theme === 'retro') {
    return (
      ...
      >
        {/* Subtle Neon Purple Glow lines inside the boundary of the frame */}
        ...
        {/* Dynamic Cursor Spotlight Overlay (Mouse Light) */}
        {isHovered && (
          <div 
  ...
  return (
    ...
    >
      {/* Subtle Neon Turquoise Glow lines inside the boundary of the frame */}
      ...
      {/* Blueprint Grid Pattern with subtle parallax shift based on tilt */}
      <div 
      ...
      {/* Content wrapper with micro-parallax and scale */}
      <div 
...
// Technical wrapper for rendering Vector Graphics with full lightboxes
...
// Interactive premium drag-drop/click image uploader specifically for user visuals
...
function ImageUploadPlaceholder({ 
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Compress with maximum resolution and quality to preserve stunning visual details (~180-350KB)
...
// Beautifully designed premium Section Transition with thin technical lines, coordinates, and slow scanning lasers
function SectionTransition({ index }: { index: number }) {
  ...
  return (
    <div className="w-full py-28 sm:py-36 md:py-44 relative flex flex-col items-center justify-center overflow-hidden select-none pointer-events-none my-16">
      ...
      {/* Background horizontal guiding grid lines */}
      ...
      {/* Animated scanning technical laser line */}
      <div 
      ...
      {/* Grid reference dot clusters / Coordinate alignment ticks */}
      ...
      {/* Minimalist Section tags & Labels */}
      <div className="relative z-10 flex flex-col items-center gap-4 text-center px-4">
        {/* Diamond traditional element matching values PDF */}
        ...
        {/* Technical Coordinate and grid reading data */}
...
function AssetsManifestProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Try to load from static assets_manifest.json in public folder first,
...
function CategoryViewInner({ categoryId, onBack }: CategoryViewProps) {
    ...
    switch (categoryId) {
      ...
      case 'magazine':
        return {
          ...
          part2: 'Dergisi',
          color: '#A78BFA',
          desc: 'Gürbüz, kaosun ortasında bir gürz ağacı mottosuyla iki ayda bir yayımlanan bir kültür, sanat ve edebiyat dergisidir. Adını bir şiirde geçen "gürz" kelimesinden alan dergimizin yayımlanmış olan toplam 5 sayısının tamamının görsel ve editoryal mizanpaj tasarımı bana aittir.'
        };
      case 'products':
        return {
  ...
  // States for custom image uploading in each sub-category
  const [aiFactoryImage, setAiFactoryImage] = usePersistentState('aiFactoryImage');
  ...
  const [diaryVizyonImage, setDiaryVizyonImage] = usePersistentState('diaryVizyonImage');
  const [diaryManifestoImage, setDiaryManifestoImage] = usePersistentState('diaryManifestoImage');
  const [diaryArkaKapakImage, setDiaryArkaKapakImage] = usePersistentState('diaryArkaKapakImage');
  const [diaryIcSayfaImage, setDiaryIcSayfaImage] = usePersistentState('diaryIcSayfaImage');
  const [backdropImage, setBackdropImage] = usePersistentState('backdropImage');

  // States for the redesigned AI Agent Factory (Pixelated Purple Theme)
  const [aiFactoryThemeImage, setAiFactoryThemeImage] = usePersistentState('aiFactoryThemeImage');
  const [aiFactoryBackdropImage, setAiFactoryBackdropImage] = usePersistentState('aiFactoryBackdropImage');
  const [aiFactoryDirectionImage, setAiFactoryDirectionImage] = usePersistentState('aiFactoryDirectionImage');
  const [aiFactoryFrameImage, setAiFactoryFrameImage] = usePersistentState('aiFactoryFrameImage');
  const [aiFactoryCarouselImage, setAiFactoryCarouselImage] = usePersistentState('aiFactoryCarouselImage');
  const [aiFactoryInstaActiveIdx, setAiFactoryInstaActiveIdx] = useState(0);
  const [aiFactoryInsta2Image, setAiFactoryInsta2Image] = usePersistentState('aiFactoryInsta2Image');
  const [aiFactoryInsta3Image, setAiFactoryInsta3Image] = usePersistentState('aiFactoryInsta3Image');
  const [aiFactoryInsta4Image, setAiFactoryInsta4Image] = usePersistentState('aiFactoryInsta4Image');
  const [aiFactoryStickerImage, setAiFactoryStickerImage] = usePersistentState('aiFactoryStickerImage');
  const [aiFactoryBadgeImage, setAiFactoryBadgeImage] = usePersistentState('aiFactoryBadgeImage');
  const [aiFactoryTrophyImage, setAiFactoryTrophyImage] = usePersistentState('aiFactoryTrophyImage');

  // States for the redesigned R&D Techathon
  const [rdThemeImage, setRdThemeImage] = usePersistentState('rdThemeImage');
  ...
  const [rdYakaKartiFrontImage, setRdYakaKartiFrontImage] = usePersistentState('rdYakaKartiFrontImage', null);
  const [rdYakaKartiBackImage, setRdYakaKartiBackImage] = usePersistentState('rdYakaKartiBackImage', null);
  const [rdVideoUrl, setRdVideoUrl] = usePersistentState('rdVideoUrl', null);
  const [rdStickerImage, setRdStickerImage] = usePersistentState('rdStickerImage', null);
  const [rdTshirtFrontImage, setRdTshirtFrontImage] = usePersistentState('rdTshirtFrontImage', null);
  const [rdTshirtBackImage, setRdTshirtBackImage] = usePersistentState('rdTshirtBackImage', null);
  const [rdDirection1Image, setRdDirection1Image] = usePersistentState('rdDirection1Image', null);
  const [rdDirection2Image, setRdDirection2Image] = usePersistentState('rdDirection2Image', null);
  const [yakaKartiSide, setYakaKartiSide] = useState<'front' | 'back'>('front');

  // States for the redesigned Eğitim/Atölye (Architecht Academy)
  const [egitimThemeImage, setEgitimThemeImage] = usePersistentState('egitimThemeImage');
  const [egitimColorPaletteImage, setEgitimColorPaletteImage] = usePersistentState('egitimColorPaletteImage');
  const [egitimTypographyImage, setEgitimTypographyImage] = usePersistentState('egitimTypographyImage');
  const [egitimKit1Image, setEgitimKit1Image] = usePersistentState('egitimKit1Image');
  const [egitimKit2Image, setEgitimKit2Image] = usePersistentState('egitimKit2Image');
  const [egitimKit3Image, setEgitimKit3Image] = usePersistentState('egitimKit3Image');
  const [egitimPleksi1Image, setEgitimPleksi1Image] = usePersistentState('egitimPleksi1Image');
  const [egitimPleksi2Image, setEgitimPleksi2Image] = usePersistentState('egitimPleksi2Image');
  const [egitimPleksi3Image, setEgitimPleksi3Image] = usePersistentState('egitimPleksi3Image');
  const [egitimPankart1Image, setEgitimPankart1Image] = usePersistentState('egitimPankart1Image');
  const [egitimPankart2Image, setEgitimPankart2Image] = usePersistentState('egitimPankart2Image');
  const [egitimPankart3Image, setEgitimPankart3Image] = usePersistentState('egitimPankart3Image');
  const [egitimPankart4Image, setEgitimPankart4Image] = usePersistentState('egitimPankart4Image');
  const [egitimPankartActiveIdx, setEgitimPankartActiveIdx] = useState(0);
  const [egitimYakaFrontImage, setEgitimYakaFrontImage] = usePersistentState('egitimYakaFrontImage');
  const [egitimYakaBackImage, setEgitimYakaBackImage] = usePersistentState('egitimYakaBackImage');
  const [egitimYakaFlipped, setEgitimYakaFlipped] = useState(false);
  const [egitimVeriNimetImage, setEgitimVeriNimetImage] = usePersistentState('egitimVeriNimetImage');

  // States for main project list card covers
  const [degerlerCoverImage, setDegerlerCoverImage] = usePersistentState('degerlerCoverImage');
  const [aiFactoryCoverImage, setAiFactoryCoverImage] = usePersistentState('aiFactoryCoverImage');
  const [rdCoverImage, setRdCoverImage] = usePersistentState('rdCoverImage');
  const [egitimCoverImage, setEgitimCoverImage] = usePersistentState('egitimCoverImage');
  const [rdTechathonCoverImage, setRdTechathonCoverImage] = usePersistentState('rdTechathonCoverImage');

  // States for social-media category
  const [socialGridImage, setSocialGridImage] = usePersistentState('socialGridImage');
  const [socialVideoImage, setSocialVideoImage] = usePersistentState('socialVideoImage');
  const [socialCampaignImage, setSocialCampaignImage] = usePersistentState('socialCampaignImage');
  const [socialGrid1Image, setSocialGrid1Image] = usePersistentState('socialGrid1Image');
  const [socialGrid2Image, setSocialGrid2Image] = usePersistentState('socialGrid2Image');
  const [socialGrid3Image, setSocialGrid3Image] = usePersistentState('socialGrid3Image');
  const [socialGrid4Image, setSocialGrid4Image] = usePersistentState('socialGrid4Image');
  const [socialGrid5Image, setSocialGrid5Image] = usePersistentState('socialGrid5Image');
  const [socialGrid6Image, setSocialGrid6Image] = usePersistentState('socialGrid6Image');
  const [socialGrid7Image, setSocialGrid7Image] = usePersistentState('socialGrid7Image');
  const [socialGrid8Image, setSocialGrid8Image] = usePersistentState('socialGrid8Image');
  const [socialGrid9Image, setSocialGrid9Image] = usePersistentState('socialGrid9Image');
  const [campaignSlide1Image, setCampaignSlide1Image] = usePersistentState('campaignSlide1Image');
  const [campaignSlide2Image, setCampaignSlide2Image] = usePersistentState('campaignSlide2Image');
  const [campaignSlide3Image, setCampaignSlide3Image] = usePersistentState('campaignSlide3Image');
  const [campaignSlide4Image, setCampaignSlide4Image] = usePersistentState('campaignSlide4Image');

  // Dynamic persistent states for sub-communities under social-media (Gastronomi, Z Burada, Veri Bilimi)
  ...
  // Custom states for the interactive Genel Tasarımlar (apps) category
  const [yakaActiveSlide, setYakaActiveSlide] = useState(0);
  const [chocolateActiveSlide, setChocolateActiveSlide] = useState(0);
  const [calendarMonth, setCalendarMonth] = useState(6); // 6 is July
  const [calendarYear, setCalendarYear] = useState(2026);
  const [calendarUserDayIndex, setCalendarUserDayIndex] = useState(0); // 0 (Pzt) to 6 (Par) aligned under Col 1
  ...
  // Sync socialMediaData.images from manifest
  useEffect(() => {
    if (isLoaded) {
  ...
  // Sync fotoromanData.images from manifest
  useEffect(() => {
    if (isLoaded) {
    ...
    setFotoromanData(prev => {
      ...
      try {
        const cleanFotoroman = {
          ...updated,
          images: {} // Do not store images/Base64 in localStorage!
        };
  ...
  // Products and Apps Category Persistent States
  ...
  // Sync productsData.images from manifest
  useEffect(() => {
    if (isLoaded) {
  ...
  // Sync appsData.images from manifest
  useEffect(() => {
    if (isLoaded) {
      setAppsData(prev => {
        const updatedImages = { ...prev.images };
  ...
  // Değerler Lansmanı specific interactive image states
  ...
  const handleLockedClick = (ref: React.RefObject<HTMLInputElement | null>) => {
    // Permanent production lock - no-op
  };

  const [editModeStr, setEditModeStr] = usePersistentState('degerlerEditMode', 'false');
  const isLocked = true;
  const [degerlerKalbiImage, setDegerlerKalbiImage] = usePersistentState('degerlerKalbiImage', null);
  const [degerlerKitiImage, setDegerlerKitiImage] = usePersistentState('degerlerKitiImage', null);
  const [degerlerKitiBannerImage, setDegerlerKitiBannerImage] = usePersistentState('degerlerKitiBannerImage');
  const [kitDefterImage, setKitDefterImage] = usePersistentState('kitDefterImage', null);
  const [kitStickerImage, setKitStickerImage] = usePersistentState('kitStickerImage', null);
  ...
  // Custom Separator / Bookmark states (6 leaders)
  const [sepFatihImage, setSepFatihImage] = usePersistentState('sepFatihImage');
  const [sepAhiImage, setSepAhiImage] = usePersistentState('sepAhiImage');
  const [sepAliImage, setSepAliImage] = usePersistentState('sepAliImage');
  const [sepYusufImage, setSepYusufImage] = usePersistentState('sepYusufImage');
  const [sepBarbarosImage, setSepBarbarosImage] = usePersistentState('sepBarbarosImage');
  const [sepCezeriImage, setSepCezeriImage] = usePersistentState('sepCezeriImage');

  // Special notebook page samples states
  const [diaryIcSayfa1Image, setDiaryIcSayfa1Image] = usePersistentState('diaryIcSayfa1Image');
  const [diaryIcSayfa2Image, setDiaryIcSayfa2Image] = usePersistentState('diaryIcSayfa2Image');

  // Lanyard (Yaka İpi) states
  const [lanyardFrontImage, setLanyardFrontImage] = usePersistentState('lanyardFrontImage', null);
  const [lanyardBackImage, setLanyardBackImage] = usePersistentState('lanyardBackImage', null);

  // Section 6 Signage & Placards states
  const [directionFrameImage, setDirectionFrameImage] = usePersistentState('directionFrameImage', null);
  ...
  // Section 6 Interactive Placards (790x1024)
  ...
  // Section separator (desenuzun) states
  ...
  // Refs for redesigned Eğitim/Atölye (Architecht Academy)
  ...
  // Refs for main project list card covers
  ...
  // Refs for redesigned AI Agent Factory (Pixelated Purple Theme)
  ...
  // Refs for redesigned R&D Techathon
  ...
  // Değerler Lansmanı custom input refs
  ...
  // Social media category file input refs
  ...
  // Magazine Category States (5 Issues, cover & spread images)
  ...
  // Sync magazineData.images from manifest
  useEffect(() => {
    if (isLoaded) {
  ...
  const handleResetIssueImages = () => {
    if (window.confirm("Bu sayının tüm yüklenmiş görsellerini sıfırlamak istediğinize emin misiniz?")) {
      setMagazineData(prev => {
        ...
        // Delete keys matching current issue
  ...
  // Scroll to top immediately when category page mounts
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'instant' as any });
  }, [selectedProjectId]);

  // Accent Color Mapping (Orange for egitim, Orange for social-media, Green for photography, TURQUOISE for others)
  ...
  // Projects list matching user intent precisely
  ...
  // Handle PDF Drag & Drop and manual upload
    ...
    return (
      <div className="w-full flex items-center justify-center my-8 md:my-12 overflow-hidden select-none pointer-events-none opacity-90">
        <svg width="100%" height="48" viewBox="0 0 1000 60" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-[800px]">
          ...
          {/* Four small orange diamond dots around Left End Diamond */}
          ...
          {/* Left Chevron / Arrow Group */}
          <path d="M 82 12 L 72 30 L 82 48" stroke="#A31D44" strokeWidth="2" fill="none" />
          <path d="M 115 12 L 95 30 L 115 48 L 122 42 L 109 30 L 122 18 Z" fill="#FF4E00" />
          <path d="M 115 15 L 140 30 L 115 45" stroke="#0062AF" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none" />
          <path d="M 122 15 L 147 30 L 122 45" stroke="#38B6E3" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />
          <path d="M 129 15 L 154 30 L 129 45" stroke="#38B6E3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />

          {/* Three parallel lines connecting Left to Center */}
          ...
          {/* Triangles resting on Left lines */}
          ...
          {/* Three parallel lines connecting Center to Right */}
          ...
          {/* Triangles resting on Right lines (Mirrored) */}
          ...
          {/* Four small orange diamond dots around Right End Diamond */}
  ...
  // Beautifully styled, authentic Repeating traditional geometric star/diamond pattern based on the PDF screenshot (now fully uploadable)
  const renderGeometricPattern = (
    ...
    return (
      <div className="w-full flex flex-col items-center justify-center my-6 relative group/divider select-none">
        ...
        {/* Decorative thin lines flanking the divider */}
  ...
  const renderIdentityGraphic = () => (
    <div className="w-full h-full flex flex-col justify-between p-4 sm:p-6 bg-gradient-to-b from-[#09090b]/90 to-black/95 rounded-2xl relative overflow-hidden select-none">
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-between">
        {/* Two Columns for Palette and Font */}
        <div className="grid grid-cols-2 gap-4 sm:gap-6 items-start mt-1">
          {/* Left Column: Renk Paleti */}
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5 mb-3">
              {/* Traditional Red & Blue Diamond */}
            ...
            {/* Color Swatch Bars stacked vertically */}
        ...
        {/* Traditional Geometric Divider Border at bottom */}
        <div className="w-full mt-2 flex justify-center">
          <svg viewBox="0 0 400 24" className="w-full h-6 opacity-95 max-w-[280px]">
            ...
            {/* Decorative chevron lines on left */}
            ...
            {/* Decorative chevron lines on right */}
            ...
            <div className="flex flex-col gap-1 w-full max-w-[160px]">
              {[
                ...
                <div 
                  ...
                  style={{ backgroundColor: swatch.hex }}
                >
                  <span className={`font-mono text-[7px] font-bold tracking-wider select-none ${swatch.hex === '#F5F7FA' ? 'text-neutral-900' : 'text-white/70 group-hover/swatch:text-white transition-colors'}`}>
                    {swatch.hex}
                  </span>
                  <span className={`font-sans text-[6px] select-none ${swatch.hex === '#F5F7FA' ? 'text-neutral-600' : 'text-white/40'} tracking-tight`}>
                    {swatch.name}
                  </span>
                </div>
              ))}
    ...
    <div className="w-full h-full flex flex-col justify-center items-center p-4 sm:p-6 bg-transparent relative overflow-hidden select-none">
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center max-w-[340px] max-h-[340px] aspect-square">
        <svg viewBox="0 0 400 400" className="w-full h-full text-[#38B6E3]">
          ...
          {/* Connected Web/Orbs of Heart Shape */}
          <g filter="url(#glow)">
            {/* Connection lines */}
      ...
      {/* Small tech coordinate captions */}
  ...
  const renderKitImages = () => (
    <div className="w-full h-full grid grid-cols-2 gap-4 p-4 sm:p-6 bg-transparent select-none">
      <div className="relative rounded-2xl overflow-hidden border border-white/5 bg-neutral-950/30 flex flex-col items-center justify-center aspect-[3/4.2] p-4 shadow-2xl group/kit0">
        {/* Vector representation of cover */}
    ...
    <div className="w-full h-full flex flex-col justify-between p-6">
      <div className="grid grid-cols-12 gap-4 items-center my-auto">
        <div className="col-span-5 flex flex-col items-center">
          {/* Vector totebag/kit box blueprint */}
    ...
    <div className="w-full h-full flex flex-col justify-between p-5">
      <div className="border border-white/5 bg-black/40 rounded-xl p-4 flex-1 flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="my-auto flex justify-center items-center h-28 relative">
          ...
          {/* Miniature Photo Frame model on side */}
    ...
    <div className="w-full h-full flex flex-col justify-between p-5">
      <div className="grid grid-cols-12 gap-4 items-center my-auto">
        <div className="col-span-4 flex justify-center">
          {/* Vertical Message Tower Totem Model */}
  ...
  // High fidelity vector graphic builders for AI Agent Factory
  ...
  const renderAiTunnelGraphic = () => (
    <div className="w-full h-full flex flex-col justify-center items-center relative overflow-hidden">
      {/* 3D wireframe grid representing the AI digital tunnel */}
  ...
  const renderGridCell = (slotKey: string, cellIndexLabel: number) => {
    ...
    if (isGastronomi) {
      ...
      // First post gets "Orman Meyveli" label on top of the visual as requested
      const tag = "";
      ...
      return (
        <div key={slotKey} className="flex flex-col items-stretch group relative p-4 bg-[#0a0a0c]/80 border border-white/5 rounded-2xl shadow-xl transition-all duration-300 hover:border-[#FF6B00]/30 hover:shadow-[0_4px_25px_rgba(255,107,0,0.05)]">
          ...
          {/* Title - below visual, centered, delicate, small font size */}
          ...
          {/* Description - fully visible, no scroll, smaller font size */}
    ...
    return (
      <div key={slotKey} className="flex flex-col items-center group relative p-3">
        ...
        {/* The main upload/display square */}
        <div 
          onClick={isLocked ? undefined : () => {
            currentUploadSlot.current = { type: 'images', key: slotKey };
            socialSingleFileInputRef.current?.click();
        ...
        {/* Editable caption with beautiful sans-serif box style */}
        <div className="w-full mt-4 px-2">
          <textarea
            ...
            placeholder="Açıklama cümlesi yazın..."
            rows={3}
            className="font-sans text-white/90 tracking-wide text-[11px] sm:text-xs text-center block w-full bg-[#121215]/80 border border-white/10 hover:border-white/20 focus:border-[#FF6B00]/40 py-2 px-3 rounded-lg transition-all focus:outline-none placeholder:text-white/30 shadow-inner resize-none min-h-[64px]"
          />
        </div>
      </div>
    );
  ...
  const renderCarousel = (prefix: string, activeIdx: number, setActiveIdx: (idx: number) => void) => {
    ...
    return (
      <div className="mt-24 border-t border-white/5 pt-16 w-full max-w-4xl mx-auto">
        ...
        {/* Slide Container - 774x530 size area as requested */}
    ...
    return (
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden opacity-25 select-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] flex items-center justify-center">
          {/* Huge faded R&D background text */}
  ...
  if (selectedProjectId === null) {
    ...
    if (categoryId === 'social-media') {
      return (
        <div className="relative min-h-screen bg-[#070707] text-white overflow-hidden">
          {/* Hidden Single File Input for uploading images */}
          <input
            type="file"
          ...
          <div className="max-w-6xl mx-auto pt-28 sm:pt-32 pb-40 px-6 sm:px-12 relative z-10 text-left">
            ...
            {/* Connection Tab bar ("çizgisel bir şey olsun, basınca ona bağlı olduğunu gösterecek şekilde") */}
            ...
            <div className="w-full">
              ...
              {activeSocialTab === 'veribilimi' && (
                <motion.div
                  ...
                  className="space-y-16"
                >
                  {/* Top Header Card with requested descriptions */}
                  ...
                  <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#0c0c0e] to-[#040405] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                      {Array.from({ length: 6 }).map((_, i) => 
                        renderGroupedImage(`veribilimi_grid_${i+1}`, i+1)
                      )}
                    </div>
                  ...
                  <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#0c0c0e] to-[#040405] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      {Array.from({ length: 9 }).map((_, i) => 
                        renderGroupedImage(`veribilimi_grid_${i+7}`, i+7)
                      )}
                    </div>
                  ...
                  {/* 3. Yapay Zeka & Veri Bilimi Zirvesi Card Group (3:4 Ratio) */}
                  <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#0c0c0e] to-[#040405] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
                    <div className="flex flex-col mb-6">
                    ...
                    {/* Group Grid of 6 images with aspect-[3/4] */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                      {Array.from({ length: 6 }).map((_, i) => 
                        renderGroupedImage(`veribilimi_grid_${i+16}`, i+16, "aspect-[3/4]")
                      )}
                    </div>
                  ...
                  <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#0c0c0e] to-[#040405] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                      {Array.from({ length: 6 }).map((_, i) => 
                        renderGroupedImage(`veribilimi_grid_${i+22}`, i+22)
                      )}
                    </div>
                    ...
                    <div className="border-t border-white/5 pt-4">
                      <p className="font-sans text-white/70 text-xs sm:text-sm leading-relaxed text-left">
                        Akbank, Türk Telekom ve ING gibi dev markaların kurumsal renklerini kendi kimlikleriyle bozmadan entegre ederken; tanışma kahvaltısı gibi daha sosyal etkinliklerde ise organik ve sıcak flat illüstrasyonlarla topluluk içi bağları güçlendirmeyi hedefledim.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="max-w-4xl mx-auto bg-gradient-to-br from-[#0c0c0e] to-[#040405] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                      {Array.from({ length: 6 }).map((_, i) => 
                        renderGroupedImage(`veribilimi_grid_${i+28}`, i+28)
                      )}
                    </div>
                    ...
                    <div className="border-t border-white/5 pt-4">
                      <p className="font-sans text-white/70 text-xs sm:text-sm leading-relaxed text-left">
                        Kaggle, Excel ve LinkedIn eğitimleri gibi odaklanmış içeriklerde; geniş yazı blokları, blok renk geçişleri ve konuşmacıyı ön plana çıkaran dairesel çerçeveler kullanarak sosyal medya akışında yüksek okunabilirlik sağlayan net bir hiyerarşi kurdum.
                      </p>
                    </div>
                  </div>
                </motion.div>
    ...
    if (categoryId === 'magazine') {
      ...
      return (
        <div className="relative min-h-screen bg-[#070707] text-white overflow-hidden animate-fadeIn">
          {/* Hidden File Input for uploading magazine images */}
          <input
            type="file"
          ...
          <div className="max-w-7xl mx-auto pt-28 sm:pt-32 pb-40 px-6 sm:px-12 relative z-10 text-left">
            ...
            {/* Description Card with Edit Mode */}
            <div className="mb-12 max-w-3xl bg-white/[0.015] border border-white/5 rounded-2xl p-6 sm:p-8 relative overflow-hidden text-left group">
              ...
              {magazineData.isEditingDesc ? (
                <textarea
                  ...
                  placeholder="Dergi ve yayın tasarımı hakkında açıklama metni giriniz..."
                />
              ) : (
            ...
            {/* Interactive Workspace Container with Sidebar & Main Reader */}
            <div className="flex flex-col lg:flex-row gap-8 items-start">
              
              {/* Left Sidebar Issue Selector */}
              <div className="w-full lg:w-[260px] shrink-0 flex flex-col gap-3">
                <span className="font-mono text-[8px] tracking-[0.25em] text-white/25 uppercase block mb-1 text-left">// EDİSYON SEÇİMİ</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="font-mono text-[7px] text-[#A78BFA] font-bold block mb-1">EDİSYON 0{issue.id}</span>
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-8 items-center w-full">
                ...
                <div className="relative w-full max-w-4xl aspect-[1.414/1] bg-black/40 border border-white/5 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.8)] overflow-hidden flex divide-x divide-white/10">
                  {/* Spine Shadow crease down the middle */}
                ...
                {/* Left/Right Previous/Next Controls */}
                ...
                <div className="w-full flex flex-col gap-3 select-none text-left">
                  ...
                  <div className="w-full grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 gap-3">
                    {Array.from({ length: currentIssueConfig.totalPages }).map((_, pIdx) => {
    ...
    if (categoryId === 'products') {
      ...
      const getPosterMetadata = (idx: number) => {
        const customTitle = productsData.titles[`products_afis_${idx + 1}`];
        const customCaption = productsData.captions[`products_afis_${idx + 1}`];
        
        const defaults = [
          {
            title: "Sessiz Ritmler (Rhythm of Silence)",
            concept: "Swiss Typographic Minimalism",
            desc: "İsviçre grafik tasarım geleneğinden ilham alan, asimetrik grid mizanpajı ve negatif alan dengesi üzerine kurulu deneysel afiş çalışması. Tipografinin salt bir bilgi aktarım aracı değil, kendi başına görsel bir enstrüman olarak kurgulandığı kompozisyon."
          },
          {
            title: "Kozmik Denge (Cosmic Equilibrium)",
            concept: "Geometric Abstraction",
            desc: "Evrensel düzen, dairesel formlar ve kozmik geometri üzerine odaklanan minimal tasarım. Güçlü kontrast renk kullanımı ve dairesel mizanpaj çizgileriyle, uzay-zaman sürekliliğini ve dengesini görsel bir forma dönüştürür."
          },
          {
            title: "Tipografik Monolog (Typographic Monologue)",
            concept: "Expressive Variable Font Study",
            desc: "Ses tonu ve konuşma ritminin değişken yazı tipleri aracılığıyla görselleştirilmesi. Kelimelerin boyutu, ağırlığı ve harfler arasındaki boşluklar, sessiz bir monoloğun içsel ritmini ve dramaturjisini yansıtır."
          },
          {
            title: "Geometrik Soyutlama (Geometric Abstraction)",
            concept: "Bauhaus Legacy & Form Analysis",
            desc: "Bauhaus okulunun form ve fonksiyon felsefesine saygı duruşu niteliğinde bir çalışma. Temel geometrik şekillerin (daire, üçgen, kare) üst üste binerek oluşturduğu yeni alanlar ve görsel derinlik araştırması."
          },
          {
            title: "Işık ve Gölge (Chiaroscuro Form)",
            concept: "Contrast & Shadow Play",
            desc: "Grafik tasarımda ışık ve gölgenin form yaratıcı gücü üzerine bir inceleme. Keskin geçişler, derin kontrastlar ve monokromatik renk paletiyle derinlik hissi yaratan minimalist kompozisyon."
          },
          {
            title: "Gürbüz Manifest (The Gürbüz Manifesto)",
            concept: "Editorial Poster Art",
            desc: "Kültür, sanat ve edebiyat dergisi Gürbüz için tasarlanan manifest afiş. Kaosun ortasındaki kararlılığı ve entelektüel duruşu simgeleyen güçlü dikey hatlar, cesur tipografi ve keskin mizanpaj kurgusu."
          },
          {
            title: "İçgüdüsel Formlar (Intuitive Forms)",
            concept: "Organic & Computational Fusion",
            desc: "Doğal akışkan formlar ile bilgisayarlı grafik algoritmalarının birleşimi. İnsan elinin içgüdüsel çizgileri ile dijital vektörlerin kusursuz simetrisinin çarpışmasını sergileyen deneysel poster."
          },
          {
            title: "Minimalist Diyalog (Minimal Dialogues)",
            concept: "Conceptual Composition",
            desc: "İki farklı formun ve rengin düzlem üzerindeki etkileşimi. Azaltılmış görsel elemanlarla en yüksek düzeyde iletişimsel berraklık ve gerilim yaratma hedefi taşıyan kavramsal tasarım projesi."
          },
          {
            title: "Akışkan Zaman (Fluid Temporality)",
            concept: "Time & Space Synthesis",
            desc: "Zamanın akışkanlığını, sarmal çizgiler ve hareket hissi veren dinamik grafik katmanlarla anlatan afiş. İzleyiciye durağan bir yüzeyde zamanın geçişini ve ritmini hissettirmeyi amaçlar."
          },
          {
            title: "Strüktürel Bauhaus (Structural Form)",
            concept: "Grid-Based Architecture",
            desc: "Mimari strüktürler ve ızgara sistemlerinin afiş mizanpajına uyarlanması. Dik açılar, yapısal kolonlar ve dengeli boşluk kullanımıyla yüzey üzerinde üç boyutlu bir mekansallık hissi."
          },
          {
            title: "Konsept İzler (Conceptual Traces)",
            concept: "Visual Memory & Texture",
            desc: "Görsel hafıza, silüetler ve dokusal katmanların birlikteliği. Yüzey üzerinde bırakılan lekeler ve silik harfler aracılığıyla, geçmiş ile gelecek, somut ile soyut arasında kurulan kavramsal bağ."
          },
          {
            title: "Zamanın Dokusu (Texture of Time)",
            concept: "Tactile Digital Poster",
            desc: "Dijital ortamda analog kağıt ve baskı dokusunun taklit edilerek oluşturulduğu duyusal afiş çalışması. Eskitilmiş dokular, asit lekeleri ve mürekkep dağılımlarıyla geçmişin izlerini bugünün tasarımıyla harmanlar."
          },
          {
            title: "Yatay Perspektifler (Horizontal Vistas)",
            concept: "Landscape Format Exhibition Art",
            desc: "Sergideki tek yatay çalışma olan bu afiş, geniş ufuk çizgileri, yatay mizanpaj mimarisi ve panoramik tipografik kurgusuyla geleneksel dikey afiş algısına meydan okuyan özgün bir sergileme deneyimi sunar."
          }
        ];

        const defaultMeta = defaults[idx] || { title: `Afiş No. ${idx + 1}`, concept: "Tasarım Seçkisi", desc: "Sergi kapsamında geliştirilen özgün tasarım çalışması." };
        
        return {
          title: customTitle || defaultMeta.title,
          concept: defaultMeta.concept,
          desc: customCaption || defaultMeta.desc
        };
      };
      ...
      const defaultsList = [
        { title: "Sessiz Ritmler (Rhythm of Silence)", desc: "İsviçre grafik tasarım geleneğinden ilham alan, asimetrik grid mizanpajı ve negatif alan dengesi üzerine kurulu deneysel afiş çalışması." },
        { title: "Kozmik Denge (Cosmic Equilibrium)", desc: "Evrensel düzen, dairesel formlar ve kozmik geometri üzerine odaklanan minimal tasarım." },
        { title: "Tipografik Monolog (Typographic Monologue)", desc: "Ses tonu ve konuşma ritminin değişken yazı tipleri aracılığıyla görselleştirilmesi." },
        { title: "Geometrik Soyutlama (Geometric Abstraction)", desc: "Bauhaus okulunun form ve fonksiyon felsefesine saygı duruşu niteliğinde bir çalışma." },
        { title: "Işık ve Gölge (Chiaroscuro Form)", desc: "Grafik tasarımda ışık ve gölgenin form yaratıcı gücü üzerine bir inceleme." },
        { title: "Gürbüz Manifest (The Gürbüz Manifesto)", desc: "Kültür, sanat ve edebiyat dergisi Gürbüz için tasarlanan manifest afiş." },
        { title: "İçgüdüsel Formlar (Intuitive Forms)", desc: "Doğal akışkan formlar ile bilgisayarlı grafik algoritmalarının birleşimi." },
        { title: "Minimalist Diyalog (Minimal Dialogues)", desc: "İki farklı formun ve rengin düzlem üzerindeki etkileşimi." },
        { title: "Akışkan Zaman (Fluid Temporality)", desc: "Zamanın akışkanlığını, sarmal çizgiler ve hareket hissi veren dinamik grafik katmanlarla anlatan afiş." },
        { title: "Strüktürel Bauhaus (Structural Form)", desc: "Mimari strüktürler ve ızgara sistemlerinin afiş mizanpajına uyarlanması." },
        { title: "Konsept İzler (Conceptual Traces)", desc: "Görsel hafıza, silüetler ve dokusal katmanların birlikteliği." },
        { title: "Zamanın Dokusu (Texture of Time)", desc: "Dijital ortamda analog kağıt ve baskı dokusunun taklit edilerek oluşturulduğu duyusal afiş çalışması." },
        { title: "Yatay Perspektifler (Horizontal Vistas)", desc: "Sergideki tek yatay çalışma olan bu afiş, geniş ufuk çizgileri, yatay mizanpaj mimarisi ve panoramik tipografik kurgusuyla geleneksel dikey afiş algısına meydan okuyan özgün bir sergileme deneyimi sunar." }
      ];

      const activeDefault = defaultsList[productsActiveIdx] || { title: `Afiş No. ${productsActiveIdx + 1}`, desc: "Sergi kapsamında geliştirilen özgün tasarım çalışması." };
      ...
      return (
        ...
        >
          {/* Hidden File Input for uploading products images */}
          <input
            type="file"
          ...
          {/* Dynamic Grid Background with Dark Gallery atmosphere */}
          ...
          {/* WELCOME / GREETING SECTION (TOP OF PAGE) */}
          <div className="max-w-7xl mx-auto px-6 sm:px-12 w-full relative z-10 flex flex-col items-start mb-4">
            ...
            {/* Title block with mixed-case elegance (not all-caps) */}
            <div className="mb-8">
            ...
            {/* Premium Welcoming / Intro Card (Bold typography summary) */}
            <div className="mb-8 max-w-3xl bg-white/[0.015] border border-white/5 rounded-2xl p-6 sm:p-8 relative overflow-hidden text-left shadow-2xl">
              ...
              <p className="font-sans text-sm sm:text-base text-white/80 leading-relaxed font-light">
                Tasarım dünyasında sınırları zorlayan, <strong>modern İsviçre tipografisinden</strong> esinlenmiş <strong>asimetrik mizanpajlar</strong>, <strong>minimalist geometrik soyutlamalar</strong> ve <strong>estetik bütünlük</strong> arayışındaki 12 dikey ve 1 yatay afiş çalışmasının yer aldığı küratöryel sergileme alanı. Her bir çalışma, görsel bir enstrüman ve kavramsal bir dil olarak kurgulanmıştır.
              </p>
            </div>
          ...
          >
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none w-full">
              ...
              {/* Lamp cylindrical body with ridges */}
            ...
            />
            {/* Soft background reflection glow on the wall */}
            ...
            <div className="max-w-7xl mx-auto px-6 sm:px-12 relative z-10 flex-1 flex flex-col justify-between w-full">
              ...
              {/* ARTWORK DISPLAY AREA & GALLERY INTERACTION */}
              <div className="relative w-full h-[400px] sm:h-[460px] md:h-[500px] flex items-center justify-center overflow-visible [perspective:1400px] my-auto">
                ...
                {/* ELEGANT NAVIGATION CHEVRONS - Placed left and right of gallery */}
                <div className="absolute left-0 sm:left-4 md:left-8 lg:left-12 z-30">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="w-11 h-11 rounded-full bg-white/[0.03] border border-white/10 hover:border-white/20 text-white/60 hover:text-white flex items-center justify-center transition-all cursor-pointer shadow-lg backdrop-blur-md"
                  >
                ...
                {/* CINEMATIC POSTER CAROUSEL (Renders all 13 posters circularized) */}
                {Array.from({ length: 13 }).map((_, idx) => {
                  const slotKey = `products_afis_${idx + 1}`;
                  const extension = (idx === 11) ? 'jpg' : 'png';
                  const imgUrl = productsData.images[slotKey] || `/uploaded/products_afis_${idx + 1}.${extension}`;
                  const isActive = idx === productsActiveIdx;
                  const isHorizontal = idx === 12; // Index 12 is the horizontal poster

                  const diff = getCircularDistance(idx, productsActiveIdx, 13);
                  const absDiff = Math.abs(diff);

                  // Five-poster carousel constraint
                  ...
                  if (isVisible) {
                    ...
                    if (diff === 0) {
                      ...
                      opacity = 0.32;
                      zIndex = 20;
                      if (isMobile) opacity = 0; // Hide outermost on small viewports
                    } else if (diff === 2) {
                      scale = 0.52;
                      rotateY = -36;
                      translateZ = -120;
                  ...
                  return (
                    ...
                    >
                      {/* GALLERY FRAME: Thick white frame with realistic shadow depth */}
                      <div 
                      ...
                      >
                        ...
                        {/* Passepartout (Beautiful heavy-weight textured white card mat) */}
                        <div className="w-full h-full bg-[#f5f5f5] p-3 sm:p-4 md:p-5 border border-gray-200/60 flex items-center justify-center relative overflow-hidden shadow-inner">
                          ...
                          {/* Artwork cutout bevel card shadow */}
                        ...
                        {/* Subtle fine print curator tag inside bottom border margin */}
                        ...
                        {/* Elegant Circular Hover-only Image Upload button to preserve CMS capability */}
                        {isActive && !isLocked && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="absolute top-4 right-4 z-30 p-2.5 rounded-full bg-black/60 hover:bg-[#FF4E20] border border-white/10 text-white/80 hover:text-white hover:scale-105 transition-all duration-300 opacity-0 group-hover:opacity-100 cursor-pointer shadow-lg backdrop-blur-sm"
              ...
              {/* BOTTOM THUMBNAILS - 12 VERTICAL + 1 HORIZONTAL (VISUALLY DISTINCT) */}
              <div className="mt-4 w-full relative z-10 border-t border-white/5 pt-5">
                ...
                <div className="flex gap-4 items-center w-full">
                  ...
                  {/* Horizontal poster thumbnail (Visually distinct landscape format) */}
                  {(() => {
                    const idx = 12;
                    const slotKey = `products_afis_${idx + 1}`;
                    const imgUrl = productsData.images[slotKey] || `/uploaded/products_afis_${idx + 1}.png`;
                    const isActive = idx === productsActiveIdx;
    ...
    if (categoryId === 'apps') {
      ...
      const yakaFrontUrl = appsData.images['apps_yaka_1'] || '/uploaded/apps_yaka_1.png' || '/uploaded/rdYakaKartiFrontImage.jpg';
      const yakaBackUrl = appsData.images['apps_yaka_2'] || '/uploaded/apps_yaka_2.jpg' || '/uploaded/rdYakaKartiBackImage.jpg';
      const chocolateBoxUrl1 = appsData.images['apps_cikolata_1'] || '/uploaded/apps_cikolata_1.jpg';
      const chocolateBoxUrl2 = appsData.images['apps_cikolata_2'] || '/uploaded/apps_cikolata_2.jpg';
      
      const baseTakvimPhotoUrl = appsData.images['apps_takvim_foto'] || '/uploaded/apps_takvim_foto.jpg' || '/uploaded/apps_takvim_foto.png';
      const takvimPhotoUrl = baseTakvimPhotoUrl 
        ? `${baseTakvimPhotoUrl}${baseTakvimPhotoUrl.includes('?') ? '&' : '?'}_t=${uploadTimestamp['apps_takvim_foto'] || ''}` 
        : '';
        
      const baseTakvimBackdropUrl = appsData.images['apps_takvim_backdrop'] || '/uploaded/apps_takvim_backdrop.png' || '/uploaded/backdropImage.jpg';
      const takvimBackdropUrl = `${baseTakvimBackdropUrl}${baseTakvimBackdropUrl.includes('?') ? '&' : '?'}_t=${uploadTimestamp['apps_takvim_backdrop'] || ''}`;

      const isCalendarMatched = calendarUserDayIndex === targetDayIndex;

      return (
        <div className="relative min-h-screen bg-[#070707] text-white overflow-hidden animate-fadeIn">
          {/* Hidden File Input for uploading apps images */}
          <input
            type="file"
          ...
          <div className="max-w-7xl mx-auto pt-28 sm:pt-32 pb-40 px-6 sm:px-12 relative z-10 text-left">
            ...
            {/* 2 Side-by-Side Cards (Kişiselleştirilmiş Yaka Kartı & Ramazan Çikolata Kutusu) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-20">
              {/* Card 1: Kişiselleştirilmiş Yaka Kartı */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <p className="font-sans text-xs sm:text-sm text-white/65 font-light leading-relaxed mb-6">
                  Kurumsal etkinlik kimliğini güçlendirmek amacıyla tasarlanan, şeffaf akrilik malzemeye özel kesim formu ve kişiye özel illüstrasyonlarla hazırlanan yaka kartı tasarımı.
                </p>

                {/* Interactive 16:9 Image Area with Slider/Carousel */}
                ...
                <div className="mt-auto pt-5 border-t border-white/5 flex flex-col gap-1.5">
                  ...
                  <p className="font-sans text-[11px] text-white/55 leading-relaxed">
                    Architecht Teknoloji Sohbetleri etkinliği için tasarlanan bu yaka kartında, standart kart anlayışının dışına çıkarak şeffaf akrilik yüzey, özel die-cut form ve katılımcıya özel dijital illüstrasyon bir araya getirildi. Etkinlik kimliğini destekleyen renk sistemi ve kurumsal grafik diliyle bütünleşen tasarım, katılımcıların yalnızca kimlik kartı değil, etkinliğin görsel deneyiminin bir parçasını taşımasını amaçladı.
                  </p>
                </div>
              </motion.div>

              {/* Card 2: Architecht 2026 Ramazan Bayramı Çikolata Kutusu Tasarımı */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <span className="font-mono text-[9px] text-[#F43F5E] tracking-[0.2em] font-bold uppercase mb-2 block">// KONSEPT AMBALAJ TASARIMI</span>
                <h3 className="font-sans font-black text-2xl sm:text-3xl text-[#00F0FF] tracking-tight leading-tight uppercase mb-1">
                ...
                <p className="font-sans text-xs sm:text-sm text-white/65 font-light leading-relaxed mb-6">
                  Kurumsal bayramlaşma sürecini güçlendirmek amacıyla tasarlanan, özel illüstrasyonlar ve marka kimliğiyle bütünleşen premium çikolata kutusu tasarımı.
                </p>

                {/* 16:9 Image Area with Carousel */}
                <div className="relative w-full aspect-[16/9] rounded-2xl overflow-hidden border border-white/10 bg-black/40 mb-6 group-hover:border-white/20 transition-all flex items-center justify-center">
                  {((chocolateActiveSlide === 0 ? chocolateBoxUrl1 : chocolateBoxUrl2)) ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-6 text-center">
                      ...
                      <span className="font-sans text-[11px] text-white/20 mt-1">
                        Görsel Seçin (16:9 Oranı)
                      </span>
                    </div>
                  )}
                ...
                {/* Description Box (Kutu İçindeki Açıklama) */}
                <div className="mt-auto pt-5 border-t border-white/5 flex flex-col gap-1.5">
                  <span className="font-mono text-[8px] text-[#F43F5E] tracking-wider uppercase font-bold">// KUTU İÇİNDEKİ AÇIKLAMA</span>
                  <p className="font-sans text-[11px] text-white/55 leading-relaxed">
                    Architecht için Ramazan Bayramı kapsamında tasarlanan bu özel hediye kutusunda; suluboya dokuları, İstanbul silüeti ve kurumsal görsel dil bir araya getirildi. Kutu tasarımının yanı sıra iç kapak mesajı ve tüm baskı yüzeyleri tek bir görsel deneyim oluşturacak şekilde tasarlandı. Amaç, kurumsal hediyeyi yalnızca bir ambalaj değil; markanın değerlerini yansıtan anlamlı bir iletişim aracına dönüştürmekti.
                  </p>
                </div>
              </motion.div>
            </div>
            ...
            <div className="mt-28 relative">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start bg-[#0a0a0f] border border-white/5 rounded-3xl p-6 sm:p-8 shadow-2xl relative mb-16">
                ...
                <div className="lg:col-span-4 bg-black/40 border border-white/5 p-6 rounded-2xl flex flex-col gap-5">
                  ...
                  <div className="flex flex-col gap-1.5 border-t border-white/5 pt-4">
                    ...
                    {appsData.images['apps_takvim_backdrop'] && (
                      <button
                        onClick={() => {
                          setAppsData(prev => {
                            const updated = {
                              ...
                              images: {
                                ...prev.images,
                                apps_takvim_backdrop: ''
                              }
                            };
                            try {
                              localStorage.setItem('apps_assets_v2', JSON.stringify({ ...updated, images: {} }));
                            } catch (err) {}
                            return updated;
                          });
                        }}
                        className="text-red-400 hover:text-red-300 font-mono text-[8px] tracking-wider uppercase text-center mt-1 cursor-pointer"
                      >
                        Orijinal Arka Plana Dön
                      </button>
                    )}
                  </div>
                  ...
                  <div className="border-t border-white/5 pt-4 flex flex-col gap-2">
                    ...
                    <div className="bg-[#00F0FF]/5 border border-[#00F0FF]/15 rounded-xl p-3">
                      ...
                      <p className="font-sans text-[10px] text-[#00F0FF]/70 mt-1 italic">
                        Mavi takvim panelini kaydırarak veya yukarıdan gün seçerek, <strong>1</strong> rakamını <strong>{WEEKDAYS[targetDayIndex]}</strong> sütununa hizalayın.
                      </p>
                    </div>
                  </div>
                </div>

                {/* Calendar Board Simulation (Right column - 8 cols) */}
                <div className="lg:col-span-8 flex flex-col items-center justify-center relative min-h-[460px] border border-white/5 rounded-2xl bg-[#08080c] overflow-hidden p-4 sm:p-6 group">
                  ...
                  {/* 3D Acrylic Board Simulation Card - Powered 1-to-1 by the backdrop image */}
                  <div 
                  ...
                  >
                    {/* Reliable real image tag for iframe sandbox and precision scaling */}
                    <img
                    ...
                    {/* Hover upload button right on the card */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        currentAppsUploadSlot.current = 'apps_takvim_backdrop';
                    ...
                    {/* INTERACTIVE COMPONENT LAYERS INSTALLED EXACTLY OVER THE EMPTY CENTRAL SPOT */}
                    {/* Month Slider Overlay Row (Matches backdrop layout perfectly) */}
                    <div className="absolute top-[13%] left-[21.5%] right-[24.5%] h-[28px] flex items-center justify-between z-10 px-2 select-none">
                      <div className="relative w-full h-full flex items-center justify-between font-sans font-semibold tracking-wider">
                        {["OCA", "ŞUB", "MAR", "NİS", "MAY", "HAZ", "TEM", "AĞU", "EYL", "EKİ", "KAS", "ARA"].map((name, i) => {
                          ...
                          return (
                            <span
                              ...
                              className={`cursor-pointer transition-all uppercase px-0.5 text-center select-none z-10 flex items-center justify-center font-bold text-[7px] sm:text-[8px] md:text-[9.5px] tracking-tight ${
                                isActive 
                        ...
                        {/* Slide/frame overlay representing the real acrylic blue cutout */}
                        <div
                    ...
                    {/* Dates Grid & Horizontal Blue Slider (Matches backdrop layout perfectly) */}
                    <div className="absolute top-[38%] bottom-[12%] left-[21.5%] right-[24.5%] overflow-hidden flex items-center z-10 p-1">
                      
                      {/* Standard printed timeless dates grid in background (13 fixed columns) */}
                      <div 
                        className="absolute left-0 right-0 select-none px-1"
                        style={{
                          display: 'grid',
                          gridTemplateColumns: 'repeat(13, minmax(0, 1fr))',
                          top: '25%', // percentage based shift to align perfectly under headers
                          height: '75%', // percentage based height
                          alignItems: 'center'
                        }}
                      >
                            ...
                            <div key={colIdx} className="flex flex-col justify-between h-[90%] my-auto text-center">
                              {columnDates.map((date, rowIdx) => {
                                ...
                                return (
                                  <div
                                    key={rowIdx}
                                    className={`font-mono text-[7px] sm:text-[8px] md:text-[9.5px] font-semibold flex-1 flex items-center justify-center transition-all duration-300 ${
                                      isUnderSlider 
                                        ? isCalendarMatched 
                      ...
                      {/* Movable Weekday Slider Panel Overlay representing the real blue acrylic frame */}
                      <div
                      ...
                      >
                        {/* Weekday headers top inside slider */}
                        <div 
                        ...
                        >
                          {["PZT", "SAL", "ÇAR", "PER", "CUM", "CMT", "PAZ"].map((day, dIdx) => (
                            <div
                              key={day}
                              className="font-sans text-[6px] sm:text-[7.5px] md:text-[9px] font-bold tracking-wider uppercase text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]"
                            >
                              {day}
                            </div>
                          ))}
                        ...
                        {/* Central transparent cutout window - frames the printed numbers underneath */}
                        <div 
              ...
              {/* 2 Explanation Boxes (MOVED TO BOTTOM OF SIMULATOR AS REQUESTED) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
                <div className="bg-[#09090d] border border-white/5 rounded-2xl p-6 hover:border-[#00F0FF]/20 transition-all duration-300">
                  ...
                  <p className="font-sans text-xs sm:text-sm text-white/60 font-light leading-relaxed">
                    Architecht yöneticileri için özel olarak geliştirilen, mekanik olarak dairesel ve yatay akrilik sürgüler yardımıyla her yıl ve her ay kullanılabilen, kağıt tüketimini sıfırlayan ve estetik duruşuyla prestij katan yenilikçi bir fiziksel masa aksesuarıdır.
                  </p>
                </div>
                ...
                <div className="bg-[#09090d] border border-white/5 rounded-2xl p-6 hover:border-[#00F0FF]/20 transition-all duration-300">
                  ...
                  <p className="font-sans text-xs sm:text-sm text-white/60 font-light leading-relaxed">
                    Yukarıda yer alan simülatördeki kontrol panelinden yıl ve ay seçimi yaptığınızda, o ayın ilk gününün (1 rakamının) hangi güne denk geldiği otomatik hesaplanır. Hemen yanındaki interaktif akrilik panelde mavi gün sürgüsünü sola ve sağa hareket ettirerek doğru hizalamayı gerçekleştirin!
                  </p>
                </div>
              </div>

              {/* ORIGINAL CALENDAR PHOTO UPLOAD AT THE BOTTOM */}
              <div className="mt-16 bg-[#0a0a0f] border border-white/5 rounded-3xl p-6 sm:p-8 flex flex-col items-start text-left relative overflow-hidden group">
                ...
                <div className="w-full aspect-[16/9] rounded-2xl overflow-hidden border border-white/10 bg-black/40 mb-6 group-hover:border-[#00F0FF]/30 transition-all flex items-center justify-center relative">
                  {takvimPhotoUrl ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-6 text-center">
                      ...
                      <span className="font-sans text-[11px] text-white/20 mt-1">
                        Sistemde var olan takvim tasarımının fiziksel maket fotoğrafını buraya yükleyin
                      </span>
                    </div>
                  )}
                ...
                <div className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-t border-white/5 pt-4">
                  <span className="font-sans text-xs text-white/50 leading-relaxed max-w-xl">
        ...
        <div className="max-w-6xl mx-auto pt-28 sm:pt-32 pb-40 px-6 sm:px-12 relative z-10 text-left">
          ...
          <div className={`grid grid-cols-1 ${categoryId === 'social-media' ? 'lg:grid-cols-3 md:grid-cols-3' : 'md:grid-cols-2'} gap-8`}>
            {projects.map((proj, index) => {
              ...
              const getProjectShortDesc = (id: string, fallback: string) => {
                const shortDescs: Record<string, string> = {
                  'degerler': 'Yenilenen misyon, vizyon ve kurumsal değerleri aktarmak amacıyla kurgulanan bütünsel görsel kimlik tasarımı.',
                  'ai-factory': 'Yapay zekâ ve otonom asistanların dünyasını interaktif ve piksel estetiğiyle buluşturan deneyim alanı.',
                  'rd': 'Kurumsal yapıyı optimize eden, yenilikçi süreç tasarımı ve derinlemesine veri analiz haritası.',
                  'egitim': 'Architecht Academy kapsamında düzenlenen eğitim programlarının kurumsal kimlik ve fiziksel deneyim tasarımı.',
                  'rd-techathon-2026': 'Geliştirici ekipler için tasarlanan etkinlik kimliği, yönlendirmeler ve vizyoner sunum tasarımı.'
                };
                return shortDescs[id] || fallback;
              };

              if (categoryId === 'organizational') {
                const coverData = getProjectCoverData(proj.id);
                const isRdTechathon = proj.id === 'rd-techathon-2026';
                return (
                  <motion.div
                    key={proj.id}
                    onClick={() => setSelectedProjectId(proj.id)}
                    whileHover={{ y: -6, borderColor: isRdTechathon ? '#FF4E00' : (proj.id === 'degerler' ? '#00F0FF' : proj.id === 'ai-factory' ? '#A86CFF' : '#FF4E00') }}
  ...
  return (
    <div className={`relative min-h-screen ${customBg} text-white overflow-hidden`}>
      ...
      {/* Glass/Film Grain Overlay */}
      <div className="paper-grain opacity-85" />

      {/* DEDICATED PRESTIGE CASE STUDY VIEW */}
      <div className="w-full pt-28 sm:pt-32 pb-40 px-6 sm:px-12 relative z-10">
        
        {/* Back and Sub-nav Navigation */}
        ...
        {/* Case Study Title Header (HUGE TYPOGRAPHY AS REQUESTED) */}
        ...
        <div className="max-w-6xl mx-auto relative z-10 mb-16">
          <div className={`w-full ${
            ...
            {/* Retro Corner Accents if AI Factory */}
            {isAiFactory && (
              <>
        ...
        {/* Dynamic Storytelling Sections (Side-by-side with visual placeholders as requested) */}
        <div className="max-w-6xl mx-auto flex flex-col gap-12 md:gap-16 relative z-10 text-left">
          {/* Storytelling sections start directly below */}

          {selectedProjectId === 'degerler' && (
            /* ==========================================
               DEĞERLER LANSMANI SECTIONS FROM THE PDF
               ========================================== */
            <>

              {/* Section 1: Tasarım Kimliği */}
              <motion.div 
                  ...
                  <div className="border-t border-white/5 pt-6 mt-2 flex flex-col gap-4">
                    <div>
                      ...
                      <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                        Lansman başlıklarında <strong className="font-semibold text-white/80">Armstrong tabanlı</strong> el yazısı detayları kullanarak geleneksel derinlik ve felsefi köprüyü yansıttım.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* Section 2: Tipografik Form & Değerler Kalbi */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  ...
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Bu tasarımda <strong className="font-semibold text-white">Güven</strong>, <strong className="font-semibold text-white">Müşteri Odaklılık</strong>, <strong className="font-semibold text-white">Çevik Takım Ruhu</strong>, <strong className="font-semibold text-white">Sürekli Gelişim</strong> ve <strong className="font-semibold text-white">Yenilikçilik</strong> kavramlarını sadece birer kelime olarak bırakmayıp, kurumun duruşunu simgeleyen geometrinin rasyonel gücüyle birleştirdim; oluşturduğum <strong className="font-semibold text-[#38B6E3]">kalp formuyla</strong> da değerlerin arkasındaki insani bağı, yaşayan ve birbirini besleyen <strong className="font-semibold text-white">bütünsel bir kültür</strong> olarak somutlaştırmaya çalıştım.
                  </p>
                  <div className="border-t border-white/5 pt-6 mt-2">
                    <h4 className="font-mono text-[8px] tracking-widest text-[#38B6E3] uppercase mb-1.5">// KURUM KÜLTÜRÜNÜN ATAN KALBİ</h4>
                    <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                      Bu çalışma, lansmanın ve kurum kültürümüzün atan kalbidir. Belirlediğim <strong className="font-semibold text-white/80">alternatif renkleri ve geometrik örüntüleri</strong>, tüm kurumsal değerlerimizi birbirine bağlayan <strong className="font-semibold text-[#38B6E3]">organik bir kalp formu</strong> oluşturacak şekilde kurguladım.
                    </p>
                  </div>
                </div>
                <div className="lg:w-7/12">
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  ...
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Lansman kitinin dış kaplamasını ve bez kesesini, etkinlik için kurguladığım alternatif renk paleti ve döngüsel örüntülerle en baştan tasarladım. Bu kapağın altında bizden simalar ve cümleler var! Kitin içindeki etkileşimi artırmak adına kurumsal logolardan oluşan dinamik bir sticker seti, anahtarlık ve broş gibi mikro tasarım nesneleri kurguladım.
                  </p>
                  <div className="border-t border-white/5 pt-6 mt-2">
                    <h4 className="font-mono text-[8px] tracking-widest text-[#38B6E3] uppercase mb-1.5">// İLK DOKUNUŞ HİSSİ</h4>
                    <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                      Amacım, katılımcının kite ilk dokunduğu andan itibaren yüksek kalitede bir markalama (branding) ve aidiyet hissetmesini sağlamaktı.
                    </p>
                  </div>
                </div>
              ...
              {/* Kit Detayları Yer Tutucu (Defter Tasarımı ve Sticker Seti) */}
              <motion.div 
                ...
                <div className="border border-white/5 bg-[#07070a]/40 backdrop-blur-md rounded-2xl p-6 sm:p-8 flex flex-col justify-between transition-all duration-300 hover:border-[#38B6E3]/20 group">
                  <div className="mb-6">
                    ...
                    <p className="font-sans text-xs text-white/50 leading-relaxed font-light">
                      Lansman kitinin iç kısmına ait genel görünüm, yerleşim kurgusu ve seperatör yerleşimleri.
                    </p>
                  </div>
              ...
              >
                ...
                <div className="max-w-4xl flex flex-col justify-center">
                  ...
                  {/* Image 2 Text */}
                  <p className="font-sans text-xs sm:text-sm text-white/70 leading-relaxed font-light mb-4 border-l-2 border-[#38B6E3]/30 pl-4 bg-[#38B6E3]/5 py-2.5 rounded-r-lg">
                    Bu defter tasarımı, lansmanın sadece bir hatıra objesi değil; <strong>kurumun kurumsal vizyonunu, misyonunu ve gelecek felsefesini içinde barındıran ana iletişim ve kimlik manifestosudur.</strong> Tasarım sürecinde sıradan, fonksiyonel bir not defteri çizmenin çok ötesine geçerek; markanın köklü kimliği ile geleceğe yön veren teknolojik gücü arasında felsefi bir köprü kurmayı hedefledim. Bu doğrultuda defterin iç mimarisini, <strong>tarihe, bilme ve teknolojiye yön vermiş karakterlerle kurum değerlerinin özdeşleştiği</strong> devasa bir kavramsal kurgu üzerine inşa ettik.
                  </p>
                </div>

                {/* Subtitle / Label for the 6 Separators */}
                <div>
                  ...
                  {/* The 6 Separator Cards Side-by-Side (Grid) */}
                    ...
                    {[
                      {
                        ...
                        text: 'Finansal teknolojide yapay zekâ dönüşümüne öncülük ederek en güvenilir iş ortağı olmak, bizim felsefemizin ve vizyonumuzun temelidir.',
                      ...
                      },
                      {
                        ...
                        motto: 'İnançla',
                        value: 'Güven',
                        text: 'Dürüstlük ve güven ilkelerimizi her şeyin üzerinde tutar; söylediğimizi yapar, yaptığımız işi büyük bir inançla sahipleniriz.',
                      ...
                      },
                      {
                        ...
                        text: 'Bugünün başarısını yarının ufkuna dönüştürür; öğrenme aşkıyla, merakla ve heyecanla kendimizi ve teknolojimizi her gün geliştiririz.',
                      ...
                      },
                      {
                        ...
                        text: 'Müşterilerimizin sadece bugünkü ihtiyaçlarını çözmekle kalmaz; bilgelikle, gelecekteki beklenti ve deneyimlerini de öngörürüz.',
                      ...
                      },
                      {
                        ...
                        text: 'Ortak hedefe doğru tek bir yürek olarak ilerler; birlikte karar alır ve zorluklara karşı çevik bir takım ruhuyla göğüs gereriz.',
                      ...
                      },
                      {
                        ...
                        motto: 'İlhamla',
                        value: 'Yenilikçilik',
                        text: 'Deneyim yolculuğunu ileri teknolojiyle güçlendirir; yenilikçi çarklarımızla sektörde kalıcı ve fark yaratan çözümler üretiriz.',
                      ...
                      return (
                        <div key={card.id} className="flex flex-col h-full group">
                          ...
                          >
                            ...
                            {/* 827x1300-sized middle image area as requested */}
                            <div 
                            ...
                            >
                              {imageVal ? (
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center p-2 text-center select-none">
                                  ...
                                  {!isLocked && (
                                    <div className="mt-1 bg-[#38B6E3]/10 border border-[#38B6E3]/20 rounded-full px-2 py-0.5 text-[#38B6E3] font-mono text-[5px] font-bold tracking-wider animate-pulse flex items-center gap-1">
                                      <Upload className="w-1.5 h-1.5" /> YÜKLE
                                    </div>
                                  )}
                                </div>
                              )}
                            ...
                            {/* Character & Value at the bottom: centered and clear */}
                          ...
                          {/* Text Below the Card as requested */}
                ...
                <div>
                  ...
                  {/* The 4 Special Cards with their texts below */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      },
                      {
                        ...
                        text: 'Yedi temel maddelik müşteri deneyim manifestosu ve tüm lider karakterlerin bir arada bulunduğu özel sayfa.',
                        detail: 'Liderlerimizin felsefesini tek bir görselde toplayan ve manifestoyu duyuran bütünsel sayfa tasarımı.',
                      ...
                      },
                      {
                        ...
                        tag: 'DEFTER İÇ SAYFA',
                        text: 'Defterin içinden 1. örnek sayfa şablonu. Geometrik kenar motifleri ve lider detayları.',
                        detail: 'Liderlerin sayfalarını ve genel yapıyı süsleyen modern-geleneksel sentezi ilk örnek sayfa düzeni.',
                      ...
                      },
                      {
                        ...
                        tag: 'DEFTER İÇ SAYFA',
                        text: 'Defterin içinden 2. örnek sayfa şablonu. Alternatif sayfa yapısı ve çizgi detayları.',
                      ...
                      return (
                        <div key={card.id} className="flex flex-col h-full group">
                          ...
                          {/* High fidelity interactive mockup frame */}
                          <div 
                            onClick={isLocked ? undefined : handleCardClick}
                          ...
                          >
                            {imageVal ? (
                              <div className="absolute inset-0 w-full h-full">
                                ...
                                {!isLocked && (
                                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col items-center justify-center gap-1.5 z-40">
                              ...
                              <>
                                ...
                                <div className="flex flex-col items-center justify-center my-auto py-1 z-10 select-none">
                                  <div className="w-11 h-11 rounded-full border border-dashed border-white/10 flex items-center justify-center bg-black/40 text-xl shadow-inner text-white/30 group-hover:text-[#00F0FF]/80 group-hover:border-[#00F0FF]/30 transition-colors">
                                    {isLocked ? card.icon : <Upload className="w-5 h-5" />}
                                  </div>
                                  ...
                                  {!isLocked && (
                                    <div className="mt-1 bg-[#38B6E3]/10 border border-[#38B6E3]/20 rounded-full px-2 py-0.5 text-[#38B6E3] font-mono text-[5px] font-bold tracking-wider animate-pulse flex items-center gap-1">
                                      <Upload className="w-1.5 h-1.5" /> GÖRSEL YÜKLE
                                    </div>
                                  )}
                                </div>
                ...
                {/* Additional detailed image display card (Image 5 text) */}
                <div className="mt-2 bg-[#07070a]/40 border border-white/5 rounded-lg p-4 backdrop-blur-md relative overflow-hidden">
                  <div className="absolute inset-0 bg-[radial-gradient(rgba(56,182,227,0.015)_1px,transparent_1px)] bg-[size:16px_16px] pointer-events-none" />
                  <div className="flex flex-col md:flex-row gap-4 items-center justify-between relative z-10">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="w-1 h-3 bg-[#38B6E3] rounded-full" />
                        <h4 className="font-mono text-[8px] text-[#38B6E3] tracking-[0.2em] font-bold uppercase">// DETAY VE TİPOGRAFİK FORM</h4>
                      </div>
                      <p className="font-sans text-[11px] sm:text-xs text-white/50 leading-relaxed font-light">
                        Her bir liderin sayfasını, o değerin ruhunu, vizyonunu ve misyonunu simgeleyen güçlü tipografiler, manifestolar ve onlara özel geliştirdiğim modern-geleneksel sentezi geometrik desenlerle işledim. Genel sayfalardaki zarif kenar süslemelerinden, arka kapaktaki \'Müşteri Deneyimi Manifestosu\'na kadar uzanan bu tasarım; Architecht’in \'Teknolojinin Mimarı, Geleceğin İmzası Sensin!\' mottosunu zamansız ve yaşayan bir marka hikayesine dönüştürmektedir.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {/* Section 5: Alan Tasarımı & Backdrop */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
              ...
              >
                {/* Section Title & Big Introductory Text */}
                <div className="max-w-4xl flex flex-col justify-center">
                  <div className="flex items-center gap-4 mb-4 select-none">
                    ...
                    <div className="flex flex-col">
                      <span className="font-mono text-[8px] text-[#38B6E3] tracking-[0.3em] font-bold uppercase mb-0.5">// DEĞERLER LANSMANI</span>
                      <h3 className="font-serif text-2xl sm:text-3xl text-white font-light tracking-tight">
                        Alan <span className="text-[#38B6E3] italic font-normal">Tasarımı</span>
                      </h3>
                    </div>
                  </div>
                  
                  {/* Introductory paragraph (Image 1 Text) */}
                  <p className="font-sans text-xs sm:text-sm text-white/70 leading-relaxed font-light mb-2 pl-4 border-l-2 border-[#38B6E3]/30 bg-[#38B6E3]/5 py-3 rounded-r-lg">
                    Bu aşamada, dijitalde ve defter tasarımında kurguladığım o derin felsefi dünyayı, katılımcıların etkileşime gireceği fiziksel etkinlik alanına taşıdım. Amacım, mekana adım atan herkesin kendini Architecht’in değerler evreninde hissetmesini sağlayacak bütünsel bir atmosfer tasarımı oluşturmaktı. Alandaki her bir ürünü bu vizyonla, işlevlerine göre ayrı ayrı kurguladım.
                  </p>
                </div>

                {/* Subtitle / Backdrop Title & Its Content Side-by-Side with Backdrop upload area */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start mt-4">
                  {/* Backdrop Description Column */}
                  <div className="lg:col-span-5 flex flex-col justify-center py-2">
                    ...
                    <p className="font-sans text-xs sm:text-sm text-white/60 leading-relaxed font-light mb-6">
                      Etkinliğin görsel merkez üssü olan backdrop tasarımında, lansmana özel belirlediğim alternatif renk paletini ve o kurumsal sürekliliği simgeleyen akışkan döngüsel örüntüleri kullandım. Büyük ölçekli bu yüzey, alanın tasarımını bütüncül olarak destekliyor ve hatıra fotoğrafı için bir alan oluşturuyor.
                    </p>

                    <div className="border-t border-white/5 pt-4 mt-2">
                      <h5 className="font-mono text-[8px] tracking-widest text-[#38B6E3] uppercase mb-1">// FOTOĞRAF ÇERÇEVESİ DETAYI</h5>
                      <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                        Katılımcıların etkinliği sosyal medyaya taşımasını ve anı taçlandırmasını sağlayan, arkasına geçilebilir büyük ölçekli bir fotoğraf alanı tasarladım. Lansmanın ana görsel kodlarını ve değer motiflerini barındıran bu çerçeve; katılımcıyı tasarımın dışından bakan bir göz olmaktan çıkarıp, bizzat o kimliğin ve 'Değerler Kalbi'nin bir parçası haline getiriyor.
                      </p>
                    </div>
                  </div>

                  {/* Backdrop Large Visual Placeholder Column */}
                  <div className="lg:col-span-7 flex flex-col justify-center w-full">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="w-1.5 h-3 bg-[#38B6E3] rounded-full" />
                      <span className="font-mono text-[8px] text-white/80 tracking-widest uppercase font-bold">// BACKDROP GÖRSEL ALANI (16:9 MOCKUP)</span>
                    </div>
              ...
              {/* Lanyard Design Section (Yaka İpi Tasarımı) */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
                ...
                <div className="max-w-4xl flex flex-col justify-center">
                  <div className="flex items-center gap-4 mb-4 select-none">
                    ...
                    <div className="flex flex-col">
                      <span className="font-mono text-[8px] text-[#38B6E3] tracking-[0.3em] font-bold uppercase mb-0.5">// DEĞERLER LANSMANI</span>
                      <h3 className="font-serif text-2xl sm:text-3xl text-white font-light tracking-tight">
                        Yaka İpi <span className="text-[#38B6E3] italic font-normal">Tasarımı</span>
                      </h3>
                    </div>
                  </div>
                  
                  <p className="font-sans text-xs sm:text-sm text-white/70 leading-relaxed font-light mb-2 pl-4 border-l-2 border-[#38B6E3]/30 bg-[#38B6E3]/5 py-3 rounded-r-lg">
                    Lansmana katılan tüm ekip üyelerinin taşıyacağı, kurum kültürümüzün ve tasarım felsefemizin boydan boya uzandığı <strong>iki farklı yaka ipi tasarımı</strong>. Tasarımlar sağdan sola, kesintisiz bir akış halinde kurgulanmıştır.
                  </p>
                </div>

                {/* Two full-width long strips for lanyard designs */}
                <div className="flex flex-col gap-6 mt-4">
                  {/* Strip 1 */}
                  <div className="w-full">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="w-1.5 h-3 bg-[#38B6E3] rounded-full" />
                      <span className="font-mono text-[8px] text-white/80 tracking-widest uppercase font-bold">// YAKA İPİ TASARIMI 1 (ÖN / DIŞ YÜZEY)</span>
                    </div>
                    <ImageUploadPlaceholder 
                      image={lanyardFrontImage}
                      onUpload={setLanyardFrontImage}
                      fileInputRef={lanyardFrontFileInputRef}
                      title="Yaka İpi Tasarımı 1 (Uzun Şerit)"
                  ...
                  <div className="w-full">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="w-1.5 h-3 bg-[#A31D44] rounded-full" />
                      <span className="font-mono text-[8px] text-white/80 tracking-widest uppercase font-bold">// YAKA İPİ TASARIMI 2 (ARKA / İÇ YÜZEY)</span>
                    </div>
                    <ImageUploadPlaceholder 
                      image={lanyardBackImage}
                      onUpload={setLanyardBackImage}
                      fileInputRef={lanyardBackFileInputRef}
                      title="Yaka İpi Tasarımı 2 (Uzun Şerit)"
                      aspect="aspect-[5316/108]"
                      objectFit="contain"
                      isLocked={isLocked}
                    />
              ...
              {/* Hand Placards (El Pankartları) Sub-Section */}
              <motion.div 
              ...
              >
                {/* Hidden inputs for interactive placards uploading */}
                <input 
                  type="file" 
                  ref={interactivePlacard1FileInputRef} 
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                      const file = e.target.files[0];
                      compressAndResizeImage(file)
                        .then(resized => {
                          setInteractivePlacard1Image(resized);
                        })
                        .finally(() => {
                          e.target.value = '';
                        });
                ...
                <input 
                  type="file" 
                  ref={interactivePlacard2FileInputRef} 
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                      const file = e.target.files[0];
                      compressAndResizeImage(file)
                        .then(resized => {
                          setInteractivePlacard2Image(resized);
                        })
                        .finally(() => {
                          e.target.value = '';
                        });
                ...
                <input 
                  type="file" 
                  ref={interactivePlacard3FileInputRef} 
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                      const file = e.target.files[0];
                      compressAndResizeImage(file)
                        .then(resized => {
                          setInteractivePlacard3Image(resized);
                        })
                        .finally(() => {
                          e.target.value = '';
                        });
                ...
                <input 
                  type="file" 
                  ref={interactivePlacard4FileInputRef} 
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                      const file = e.target.files[0];
                      compressAndResizeImage(file)
                        .then(resized => {
                          setInteractivePlacard4Image(resized);
                        })
                        .finally(() => {
                          e.target.value = '';
                        });
                ...
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h4 className="font-mono text-[10px] text-white/95 tracking-[0.2em] font-bold uppercase">// EL PANKARTLARI VE ETKİLEŞİM</h4>
                </div>

                {/* Side-by-Side Product Gallery Layout */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-10">
                  {/* Left Column: Big Active Image & Text Info below (solda görsel var yazı var) */}
                  <div className="lg:col-span-7 flex flex-col gap-6">
                    <div className="w-full max-w-[420px] mx-auto relative border border-white/10 bg-[#07070a]/60 backdrop-blur-md rounded-2xl p-5 group overflow-hidden">
                      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(0,240,255,0.01),transparent_60%)] pointer-events-none" />
                      
                      <div className="absolute -top-3 -left-3 font-mono text-[7px] text-[#00F0FF] uppercase z-20 bg-black px-2 py-0.5 rounded border border-[#00F0FF]/30 tracking-wider">
                        Aktif Görünüm // 790x1024
                      </div>
                      
                      {(() => {
                        const activeData = activeInteractivePlacard === 1 ? { img: interactivePlacard1Image, setImg: setInteractivePlacard1Image, ref: interactivePlacard1FileInputRef, title: "Model 01" } :
                                           activeInteractivePlacard === 2 ? { img: interactivePlacard2Image, setImg: setInteractivePlacard2Image, ref: interactivePlacard2FileInputRef, title: "Model 02" } :
                                           activeInteractivePlacard === 3 ? { img: interactivePlacard3Image, setImg: setInteractivePlacard3Image, ref: interactivePlacard3FileInputRef, title: "Model 03" } :
                                           { img: interactivePlacard4Image, setImg: setInteractivePlacard4Image, ref: interactivePlacard4FileInputRef, title: "Model 04" };
                        
                        return (
                          <div className="relative">
                            <ImageUploadPlaceholder 
                              image={activeData.img}
                              onUpload={activeData.setImg}
                              fileInputRef={activeData.ref}
                      ...
                      {/* Active Info Directly Below the Big Image */}
                      <div className="mt-4 pt-3 border-t border-white/5 flex flex-col gap-1.5 relative z-10 text-center items-center">
                        <span className="font-mono text-[#A31D44] text-[7.5px] tracking-widest font-bold uppercase block">
                          // AKTİF SEÇİM
                        </span>
                        <h3 className="font-serif italic text-[#38B6E3] font-light text-lg tracking-wide">
                          {activeInteractivePlacard === 1 ? "Model 01" :
                           activeInteractivePlacard === 2 ? "Model 02" :
                           activeInteractivePlacard === 3 ? "Model 03" :
                           "Model 04"}
                        </h3>
                      </div>
                    </div>
                  ...
                  {/* Right Column: Smaller Thumbnail Displays next to it (clothing site product gallery list style) */}
                  <div className="lg:col-span-5 flex flex-col gap-4">
                    <div className="flex items-center gap-2 pb-2 border-b border-white/5">
                      <span className="w-1.5 h-3 bg-[#38B6E3] rounded-full" />
                      <h4 className="font-mono text-[8.5px] text-white/90 tracking-[0.2em] font-bold uppercase">// ALTERNATİF PANKART SEÇENEKLERİ</h4>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      {[
                        { id: 1, img: interactivePlacard1Image, setImg: setInteractivePlacard1Image, ref: interactivePlacard1FileInputRef, label: "Model 01", title: "Değerler Manifestosu" },
                        { id: 2, img: interactivePlacard2Image, setImg: setInteractivePlacard2Image, ref: interactivePlacard2FileInputRef, label: "Model 02", title: "Gelecek Vizyonu" },
                        { id: 3, img: interactivePlacard3Image, setImg: setInteractivePlacard3Image, ref: interactivePlacard3FileInputRef, label: "Model 03", title: "Sinerji & Adaptasyon" },
                        { id: 4, img: interactivePlacard4Image, setImg: setInteractivePlacard4Image, ref: interactivePlacard4FileInputRef, label: "Model 04", title: "Lansman Manifestosu" }
                      ].map((item) => {
                        const isActive = activeInteractivePlacard === item.id;
                        return (
                          <div 
                            key={item.id} 
                            className="flex flex-col gap-2 group/thumbnail cursor-pointer"
                            onClick={() => setActiveInteractivePlacard(item.id)}
                          >
                            <div className={`relative rounded-xl border overflow-hidden transition-all duration-300 ${
                              isActive 
                                ? 'border-[#00F0FF] shadow-[0_0_15px_rgba(0,240,255,0.25)] scale-[1.02] bg-[#00F0FF]/5' 
                                : 'border-white/10 hover:border-white/20 bg-black/40 hover:scale-[1.01]'
                            }`}>
                              <div className="aspect-[790/1024] relative flex items-center justify-center">
                                {item.img ? (
                                  <img 
                                    src={item.img} 
                                    alt={item.label} 
                                  ...
                                  <div className="flex flex-col items-center gap-1.5 text-center p-3 text-white/30 group-hover/thumbnail:text-white/50 transition-colors">
                                    <Upload className="w-3.5 h-3.5 animate-pulse" />
                                    <span className="font-mono text-[7px] uppercase">{item.label} Boş</span>
                                  </div>
                                )}
                                
                                {/* Quick click feedback overlays */}
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/thumbnail:opacity-100 transition-opacity flex items-center justify-center">
                                  <span className="bg-black/95 text-[7px] text-white font-mono tracking-widest px-2.5 py-1 rounded border border-white/10 uppercase">
                                    {isActive ? 'Aktif Görünüm' : 'Büyüt / Seç'}
                                  </span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex justify-between items-center px-1">
                              <span className={`font-mono text-[7.5px] uppercase tracking-wider ${isActive ? 'text-[#00F0FF] font-bold' : 'text-white/50'}`}>
                                {item.label}
                              </span>
              ...
              {/* Extra divider area as requested */}
          ...
          {selectedProjectId === 'social-grid' && (
            <>
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  <span className="font-mono text-[#FF6B00] text-[10px] tracking-[0.3em] font-black uppercase mb-4 block">// 01 / ESTETİK BÜTÜNLÜK & CURATION</span>
                  <h2 className="font-sans font-black text-4xl sm:text-5xl md:text-6xl text-white uppercase tracking-tight leading-none mb-6">
                    KÜRATÖRLÜ<br/>
                    <span className="text-[#FF6B00]">GRID SİSTEMİ</span>
                  </h2>
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Sosyal medyadaki ilk izlenim, marka kimliğinin dijital evrendeki yansımasıdır. This grid tasarımı; asimetrik dengeler, nefes alan beyaz boşluklar ve minimal tipografi kullanarak markanızı standart şablonlardan ayırır.
                  </p>
                  <div className="border-t border-white/5 pt-6 mt-2">
                    <h4 className="font-mono text-[8px] tracking-widest text-[#FF6B00] uppercase mb-1.5">// GRID YAPISI VE RİTM</h4>
                    <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                      Her üç görselde bir tekrarlanan asimetrik şablonlar, renk bütünlüğü ve minimal tasarım dili ile akıcı ve estetik bir akış hiyerarşisi elde edilir.
                    </p>
                  </div>
                </div>
                <div className="lg:w-7/12 flex flex-col justify-center">
                  <ImageUploadPlaceholder 
                    image={socialGridImage}
                    onUpload={setSocialGridImage}
                    fileInputRef={socialGridFileInputRef}
                    title="Grid Tasarım Önizleme"
                    isLocked={isLocked}
                    onLockedClick={() => handleLockedClick(socialGridFileInputRef)}
                  />
                </div>
              </motion.div>
              ...
              <motion.div 
                ...
                viewport={{ once: true, margin: "-120px" }}
                transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
                className="flex flex-col lg:flex-row-reverse items-stretch gap-8 lg:gap-16"
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  <span className="font-mono text-[#FF6B00] text-[10px] tracking-[0.3em] font-black uppercase mb-4 block">// 02 / İNTERAKTİF SİMÜLATÖR</span>
                  <h3 className="font-sans font-black text-xl sm:text-2xl text-white tracking-tight uppercase mb-6">Canlı Grid Akış Testi</h3>
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Kendi görsellerinizi tek tek yükleyerek 3x3'lük bir Instagram ızgarasının nasıl görüneceğini canlı olarak test edin. Sol üstten sağ alta doğru estetik hiyerarşiyi tasarlayın ve ritmi yakalayın.
                  </p>
                  <div className="text-[10px] font-mono text-white/30 flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#FF6B00] animate-ping" />
                    <span>Hücrelere tıklayarak görsel yükleyebilirsiniz.</span>
                  </div>
                </div>
                
                <div className="lg:w-7/12 flex items-center justify-center">
                  <div className="w-full max-w-[440px] border border-white/10 bg-neutral-950/40 p-6 rounded-2xl shadow-2xl relative overflow-hidden">
                    <div className="flex justify-between items-center mb-4 pb-3 border-b border-white/5">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-pink-500 to-yellow-500 flex items-center justify-center p-0.5">
                          <div className="w-full h-full rounded-full bg-black flex items-center justify-center text-[8px] font-bold text-white font-mono">D</div>
                        </div>
                        <span className="font-mono text-[9px] text-white/80 font-bold uppercase tracking-wider">@design.showcase</span>
                      </div>
                      <span className="font-mono text-[8px] text-[#FF6B00]/60">// FEED PREVIEWER</span>
                    </div>

                    <div className="grid grid-cols-3 gap-2.5 aspect-square max-w-[360px] mx-auto">
                      {[
                        { img: socialGrid1Image, setImg: setSocialGrid1Image, ref: socialGrid1FileInputRef },
                        { img: socialGrid2Image, setImg: setSocialGrid2Image, ref: socialGrid2FileInputRef },
                        { img: socialGrid3Image, setImg: setSocialGrid3Image, ref: socialGrid3FileInputRef },
                        { img: socialGrid4Image, setImg: setSocialGrid4Image, ref: socialGrid4FileInputRef },
                        { img: socialGrid5Image, setImg: setSocialGrid5Image, ref: socialGrid5FileInputRef },
                        { img: socialGrid6Image, setImg: setSocialGrid6Image, ref: socialGrid6FileInputRef },
                        { img: socialGrid7Image, setImg: setSocialGrid7Image, ref: socialGrid7FileInputRef },
                        { img: socialGrid8Image, setImg: setSocialGrid8Image, ref: socialGrid8FileInputRef },
                        { img: socialGrid9Image, setImg: setSocialGrid9Image, ref: socialGrid9FileInputRef }
                      ].map((cell, idx) => (
                        <div 
                          key={idx} 
                          onClick={isLocked ? undefined : () => cell.ref.current?.click()}
                          className={`relative aspect-square border border-white/5 bg-white/[0.02] ${isLocked ? 'cursor-default' : 'hover:bg-white/[0.05] hover:border-[#FF6B00]/30 cursor-pointer'} transition-all duration-300 rounded-lg overflow-hidden group/cell`}
                        >
                          {!isLocked && (
                            <input 
                              type="file" 
                          ...
                          {cell.img ? (
                            <img 
                              src={cell.img} 
                              alt={`Grid ${idx+1}`} 
                              className={`w-full h-full object-cover ${isLocked ? '' : 'group-hover/cell:scale-105'} transition-transform duration-500`} 
                              referrerPolicy="no-referrer"
                            />
                          ) : (
                            <div className={`absolute inset-0 flex flex-col items-center justify-center gap-1 ${isLocked ? 'opacity-20' : 'opacity-40 group-hover/cell:opacity-100'} transition-opacity`}>
                              {!isLocked && <Upload className="w-3.5 h-3.5 text-white/40 group-hover/cell:text-[#FF6B00] transition-colors" />}
                              <span className="font-mono text-[7px] text-white/30 group-hover/cell:text-white/60">{isLocked ? "BOŞ" : `SEÇ ${idx+1}`}</span>
                            </div>
                          )}
                        </div>
                      ))}
          ...
          {selectedProjectId === 'social-video' && (
            <>
              {/* Section 1: Intro to Dynamic Video */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  <span className="font-mono text-[#FF6B00] text-[10px] tracking-[0.3em] font-black uppercase mb-4 block">// 01 / DİKEY HAREKET & RİTM</span>
                  <h2 className="font-sans font-black text-4xl sm:text-5xl md:text-6xl text-white uppercase tracking-tight leading-none mb-6">
                    DİNAMİK VİDEO<br/>
                    <span className="text-[#FF6B00]">KAPSÜLLERİ</span>
                  </h2>
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Günümüz tüketim alışkanlıklarında dikey videolar en yüksek etkileşim oranını sağlar. 9:16 oranındaki video kurgularımız, kinetik tipografi, ritmik ses senkronu ve marka estetiğini bir araya getirerek izleyicileri ekrana bağlar.
                  </p>
                  <div className="border-t border-white/5 pt-6 mt-2">
                    <h4 className="font-mono text-[8px] tracking-widest text-[#FF6B00] uppercase mb-1.5">// KİNETİK TİPOGRAFİ DESTEĞİ</h4>
                    <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                      Videolarımızın tamamı, ses kapalıyken dahi mesajı en estetik biçimde aktarabilmesi adına özelleştirilmiş hareketli altyazı ve tipografi katmanlarıyla tasarlanır.
                    </p>
                  </div>
                </div>
                <div className="lg:w-7/12 flex flex-col justify-center">
                  <ImageUploadPlaceholder 
                    image={socialVideoImage}
                    onUpload={setSocialVideoImage}
                    fileInputRef={socialVideoFileInputRef}
                    title="Dikey Video Önizleme"
                    isLocked={isLocked}
                    onLockedClick={() => handleLockedClick(socialVideoFileInputRef)}
                  />
                </div>
              </motion.div>
              ...
              {/* Section 2: Video Details & Interactive Reel Simulator */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  <span className="font-mono text-[#FF6B00] text-[10px] tracking-[0.3em] font-black uppercase mb-4 block">// 02 / REELS SIMULATOR</span>
                  <h3 className="font-sans font-black text-xl sm:text-2xl text-white tracking-tight uppercase mb-6">Mobil Reels Etkileşim Testi</h3>
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Mobil cihaz simülatöründe videonuzun dikey formda, kullanıcı arayüz elemanlarıyla birlikte nasıl duracağını görün. Sağdaki etkileşim butonlarını kullanarak beğenme simülasyonunu test edin!
                  </p>
                  <div className="flex items-center gap-4 mt-2">
                    <button 
                      onClick={() => {
                        setIsSocialVideoLiked(!isSocialVideoLiked);
                        setSocialVideoLikes(prev => isSocialVideoLiked ? prev - 1 : prev + 1);
                      }}
                      className={`flex items-center gap-2 font-mono text-[9px] tracking-widest uppercase font-bold py-2.5 px-5 rounded-lg border transition-all duration-300 ${
                        isSocialVideoLiked 
                          ? 'bg-red-500/10 border-red-500/30 text-red-400 shadow-[0_0_15px_rgba(239,68,68,0.2)]'
                          : 'bg-white/5 border-white/10 text-white/60 hover:text-white hover:border-white/20'
                      }`}
                    >
                      <span className="text-[12px]">{isSocialVideoLiked ? '❤️' : '🤍'}</span>
                      <span>{isSocialVideoLiked ? 'BEĞENİLDİ' : 'BEĞEN'}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="lg:w-7/12 flex items-center justify-center">
                  ...
                  <div className="w-[280px] h-[500px] border-[6px] border-[#1d1d1f] bg-black rounded-[36px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="absolute top-2.5 left-1/2 -translate-x-1/2 w-20 h-4 bg-black rounded-full z-30 flex items-center justify-center">
                      <div className="w-2 h-2 rounded-full bg-neutral-900 ml-auto mr-3 border border-neutral-800" />
                    </div>

                    {/* Content Layer */}
                    <div className="absolute inset-0 z-10 flex flex-col justify-between p-4 pt-10">
                      {/* Top Bar */}
                      <div className="flex justify-between items-center text-white/95 font-mono text-[7px] font-bold uppercase tracking-wider select-none bg-gradient-to-b from-black/50 to-transparent p-2 rounded-t-xl">
                        <span>Reels</span>
                        <div className="flex items-center gap-1.5">
                      ...
                      <div className="absolute inset-0 z-0">
                        {socialVideoImage ? (
                          <img 
                            src={socialVideoImage} 
                            alt="Reels mock cover" 
                            className="w-full h-full object-cover"
                            referrerPolicy="no-referrer"
                          />
                        ) : (
                          <div className="w-full h-full bg-neutral-900 flex flex-col items-center justify-center gap-3 p-4">
                            <span className="text-[20px] opacity-40">🎬</span>
                            <span className="font-mono text-[7px] text-white/30 text-center">GÖRSEL YÜKLEYEREK ÖNİZLEME YAPIN</span>
                          </div>
                        )}
                      </div>

                      {/* Reels Interaction Sidebar */}
                      <div className="absolute right-3 bottom-16 z-20 flex flex-col items-center gap-4 text-white">
                        ...
                        >
                          <div className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-md border border-white/10 flex items-center justify-center group-hover/icon:scale-110 transition-transform">
                            <span className="text-[12px]">{isSocialVideoLiked ? '❤️' : '🤍'}</span>
                          </div>
                          <span className="font-mono text-[7px] text-white/90 font-bold select-none">{socialVideoLikes}</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-1 cursor-pointer">
                          ...
                          <span className="font-mono text-[7px] text-white/90 font-bold select-none">42</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-1 cursor-pointer">
                          ...
                          <span className="font-mono text-[7px] text-white/90 font-bold select-none">105</span>
                        </div>
                      </div>

                      {/* Bottom Description Overlay */}
                      <div className="mt-auto z-20 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3 rounded-b-2xl select-none text-left">
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <div className="w-4 h-4 rounded-full bg-gradient-to-tr from-cyan-400 to-blue-500 flex items-center justify-center text-[6px] font-bold text-black font-mono">D</div>
                          <span className="font-mono text-[8px] text-white font-bold tracking-wide">@design.showcase</span>
                        </div>
                        <p className="font-sans text-[7px] text-white/85 leading-relaxed font-light line-clamp-2">
                          Markaların dikey ritmini yansıtan kreatif geçişler ve asimetrik motion kurgular. #motion #creative
                        </p>
                        <div className="flex items-center gap-1 mt-2">
                          <span className="text-[6px]">🎵</span>
                          <span className="font-mono text-[6px] text-[#FF6B00]/90 font-bold tracking-wider uppercase animate-pulse">Orijinal Ses // Dynamic Music</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {selectedProjectId === 'social-campaign' && (
            <>
              {/* Section 1: Intro to Campaign */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  <span className="font-mono text-[#FF6B00] text-[10px] tracking-[0.3em] font-black uppercase mb-4 block">// 01 / KONSEPT & HİKAYE</span>
                  <h2 className="font-sans font-black text-4xl sm:text-5xl md:text-6xl text-white uppercase tracking-tight leading-none mb-6">
                    KAMPANYA<br/>
                    <span className="text-[#FF6B00]">KURGULARI</span>
                  </h2>
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Lansmanlar ve dönemsel kampanyalar için tasarladığımız konsept görsel serileri. Sadece tekil paylaşımlar değil, kullanıcının kaydırarak okuduğu zengin hikayeli "carousel" (kaydırmalı) tasarımlar, marka derinliğini zenginleştirir.
                  </p>
                  <div className="border-t border-white/5 pt-6 mt-2">
                    <h4 className="font-mono text-[8px] tracking-widest text-[#FF6B00] uppercase mb-1.5">// KAMPANYA ETKİLEŞİMİ</h4>
                    <p className="font-sans text-xs text-white/45 leading-relaxed font-light">
                      Kullanıcıyı kaydırmaya teşvik eden, kesintisiz devam eden görsel akış şablonları ve çarpıcı, akılda kalıcı tipografik başlık tasarımları.
                    </p>
                  </div>
                </div>
                <div className="lg:w-7/12 flex flex-col justify-center">
                  <ImageUploadPlaceholder 
                    ...
                    onUpload={setSocialCampaignImage}
                    fileInputRef={socialCampaignFileInputRef}
                    title="Kampanya Ana Görseli"
                    isLocked={isLocked}
                    onLockedClick={() => handleLockedClick(socialCampaignFileInputRef)}
                  />
                </div>
              </motion.div>
              ...
              {/* Section 2: Carousel Product Gallery Style layout */}
              <motion.div 
                initial={{ opacity: 0, y: 40, filter: 'blur(8px)' }}
                whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                viewport={{ once: true, margin: "-120px" }}
              ...
              >
                <div className="lg:w-5/12 flex flex-col justify-center py-6">
                  <span className="font-mono text-[#FF6B00] text-[10px] tracking-[0.3em] font-black uppercase mb-4 block">// 02 / ÇOKLU KAMPANYA SLAYTLARI</span>
                  <h3 className="font-sans font-black text-xl sm:text-2xl text-white tracking-tight uppercase mb-6">Kampanya Slayt Serisi</h3>
                  <p className="font-sans text-sm sm:text-base text-white/60 leading-relaxed font-light mb-6">
                    Kıyafet sitelerindeki ürün galerisi tarzında tasarlanmış çoklu slayt yapısı! Soldaki küçük resimlere basarak büyük görseli değiştirebilirsiniz. Hücrelerdeki görseli değiştirmek için büyük görselin üzerine tıklayarak kendi görsellerinizi yükleyebilirsiniz!
                  </p>
                  <div className="text-[10px] font-mono text-white/30 flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#FF6B00] animate-pulse" />
                    <span>Seçilen slaytı güncellemek için büyük resme tıklayın.</span>
                  </div>
                </div>

                <div className="lg:w-7/12 flex flex-col md:flex-row gap-6">
                  {/* Small vertical thumbnails left */}
                  <div className="flex md:flex-col flex-row gap-3 order-2 md:order-1 justify-center">
                    {[
                      { id: 1, img: campaignSlide1Image, title: "Slayt 1" },
                      { id: 2, img: campaignSlide2Image, title: "Slayt 2" },
                      { id: 3, img: campaignSlide3Image, title: "Slayt 3" },
                      { id: 4, img: campaignSlide4Image, title: "Slayt 4" }
                    ].map((item) => {
                      const isActive = activeCampaignSlide === item.id;
                      return (
                        <div
                          key={item.id}
                          onClick={() => setActiveCampaignSlide(item.id)}
                          className={`w-16 h-16 md:w-20 md:h-20 rounded-xl cursor-pointer overflow-hidden border transition-all duration-300 relative group/thumb ${
                            isActive 
                              ? 'border-[#FF6B00] shadow-[0_0_12px_rgba(255,107,0,0.25)] scale-[1.03]' 
                        ...
                        >
                          {item.img ? (
                            ...
                            <div className="w-full h-full bg-white/[0.02] flex flex-col items-center justify-center">
                              ...
                              <span className="font-mono text-[6px] text-white/20">S{item.id}</span>
                            </div>
                          )}
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/thumb:opacity-100 flex items-center justify-center transition-opacity">
                            <span className="font-mono text-[7px] text-[#FF6B00] font-bold">AÇ</span>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  {/* Big Active Image Area right */}
                  <div className="flex-1 order-1 md:order-2">
                    {(() => {
                      const activeData = activeCampaignSlide === 1 ? { img: campaignSlide1Image, setImg: setCampaignSlide1Image, ref: campaignSlide1FileInputRef, title: "Slayt 01" } :
                                         activeCampaignSlide === 2 ? { img: campaignSlide2Image, setImg: setCampaignSlide2Image, ref: campaignSlide2FileInputRef, title: "Slayt 02" } :
                                         activeCampaignSlide === 3 ? { img: campaignSlide3Image, setImg: setCampaignSlide3Image, ref: campaignSlide3FileInputRef, title: "Slayt 03" } :
                                         { img: campaignSlide4Image, setImg: setCampaignSlide4Image, ref: campaignSlide4FileInputRef, title: "Slayt 04" };
                      ...
                      return (
                        <div className="w-full h-full animate-fadeIn">
                          <ImageUploadPlaceholder
                            ...
                            onUpload={activeData.setImg}
                            fileInputRef={activeData.ref}
                            title={`${activeData.title} Görseli`}
                            isLocked={isLocked}
                            onLockedClick={() => handleLockedClick(activeData.ref)}
                          />
                        </div>
                      );
                    })()}
        ...
        <div className="max-w-6xl mx-auto border-t border-white/5 pt-16 mt-20 text-center relative z-10 flex flex-col items-center gap-6">
          <div className="flex flex-col items-center gap-2 select-none">
            <span className="font-mono text-[8px] tracking-[0.4em] text-[#FF6B00]/40 uppercase">// END_OF_GALLERY</span>
            <span className="font-serif italic text-white/50 text-sm">Tüm Çalışmaları İncelediniz</span>
          </div>
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05, y: -2 }}
            whileTap={{ scale: 0.95 }}
            className="inline-flex items-center gap-2 group cursor-pointer bg-white/5 border border-white/10 text-[#FF6B00] hover:text-[#FF6B00] hover:border-[#FF6B00]/45 hover:bg-[#FF6B00]/5 font-bold font-mono text-[9px] tracking-widest uppercase py-3.5 px-8 rounded-xl transition-all duration-300 shadow-[0_0_15px_rgba(255,107,0,0.05)]"
          >
            <ArrowUp className="w-3.5 h-3.5 transition-transform group-hover:-translate-y-0.5 text-[#FF6B00]" />
            <span>Başa Dön</span>
          </motion.button>
          
          <div className="text-[6.5px] font-mono text-white/15 tracking-widest uppercase mt-4">
            LED KIOSK INTEGRATION PLATFORM VERSION 4.0 // 2026 ARCHITECT SHOWCASE
          </div>
        </div>
        ...
        {toastMessage && (
          <div className="fixed bottom-6 right-6 z-50 animate-fade-in pointer-events-none">
            <div className="flex items-center gap-3 bg-[#0c0c10]/95 backdrop-blur-md border border-[#38B6E3]/40 text-white font-sans text-xs px-4 py-3.5 rounded-xl shadow-[0_0_25px_rgba(56,182,227,0.25)]">
              <span className="w-2 h-2 rounded-full bg-[#38B6E3] animate-ping" />
              <span className="font-mono text-[9px] tracking-widest font-bold text-[#38B6E3] uppercase">// SYSTEM_ALERT:</span>
              <span className="font-mono text-[9.5px] tracking-wide font-medium text-white/90">{toastMessage}</span>
            </div>
          </div>
        )}
